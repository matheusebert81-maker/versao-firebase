...
export default function VisualEditAgent() {
	...
	const hoverOverlaysRef = useRef<any[]>([]); // Multiple overlays for hover
	const selectedOverlaysRef = useRef<any[]>([]); // Multiple overlays for selection
	const currentHighlightedElementsRef = useRef<any[]>([]); // Multiple elements for hover
	...
	const positionOverlay = (overlay: any, element: any, isSelected = false) => {
	...
	const findElementsById = (id: string, useSourceLocation = false) => {
		if (!id) return [];
	...
	useEffect(() => {
		...
		elementsWithLineNumber.forEach((el: any, index) => {
